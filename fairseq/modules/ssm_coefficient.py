...
class CoeffCalculator:
    def __init__(self, L):
        ...
        self.final_coeffs_matrix_horizontal = torch.zeros(L * L, L * L * 8).cuda()
        self.final_coeffs_matrix_vertical = torch.zeros(L * L, L * L * 8).cuda()
    ...
    def calc_coeffs_lazy(self):
        if os.path.exists(self.horizontal_cache_location) and os.path.exists(self.vertical_cache_location):
            ...
            if horizontal.shape[0] == self.L * self.L:
                self.final_coeffs_matrix_horizontal = horizontal.cuda()
                self.final_coeffs_matrix_vertical = torch.load(self.vertical_cache_location).cuda()
                return
